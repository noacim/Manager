...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kayıt <@Noâ/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
   ...
   const tagModedata = await regstats.findOne({ guildID: message.guild.id })
    ...
    if(!uye.roles.cache.has("") && !uye.roles.cache.has(ayar.vipRole) && !uye.roles.cache.has(ayar.boosterRolu)) return message.lineReply(embed.setDescription(`${uye.toString()} isimli üyenin kullanıcı adında tagımız (\`✰\`) olmadığı, <@&${ayar.boosterRolu}>, <@&${ayar.vipRole}> Rolü olmadığı için isim değiştirmekden başka kayıt işlemi yapamazsınız.`));
    }
